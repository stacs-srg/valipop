...
public class BasicStatistics extends Object
{
     ...
     * @param alpha the alpha'th quantile.
     * @param beta the beta'th quantile.
     * @param data the input data.
     ...
     *                  interest. For instance, normal distribution:
     *                  parameter[0]=mean; parameter[1]=variance.
    ...
    public double distribution(DistributionType distribution,
                               double[] parameter,
                               double[] range)
    {
        ...
        switch (distribution)
        {
            case NORMAL:
                NormalDistribution normalDistribution = new
                        NormalDistribution(parameter[0], parameter[1]);
                distValue = normalDistribution.cumulative(range[1]) -
                            normalDistribution.cumulative(range[0]);
                break;
            case BETA:
                BetaDistribution betaDistribution =
                    new BetaDistribution(parameter[0], parameter[1]);
                distValue = betaDistribution.cumulative(range[1]) -
            ...
            case BINOMIAL:
                ...
                distValue = binomialDistribution.probability(range[0]) *
                            equalInd +
                            binomialDistribution.cumulative(range[1]) -
                            binomialDistribution.cumulative(range[0]);
                break;
            case CAUCHY:
                CauchyDistribution cauchyDistribution = new CauchyDistribution(
                    parameter[0], parameter[1]);
                distValue = cauchyDistribution.cumulative(range[1]) -
                            cauchyDistribution.cumulative(range[0]);
                break;
            case CHISQUARE:
                ChiSqrDistribution chisqrDistribution = new ChiSqrDistribution(
                    parameter[0]);
                distValue = chisqrDistribution.cumulative(range[1]) -
                            chisqrDistribution.cumulative(range[0]);
                break;
            case EXPONENTIAL:
                ExponentialDistribution exponentialDistribution =
                    new ExponentialDistribution(parameter[0]);
                distValue = exponentialDistribution.cumulative(range[1]) -
                            exponentialDistribution.cumulative(range[0]);
                break;
            case F:
                FDistribution fDistribution = new FDistribution(parameter[0],
                        parameter[1]);
                distValue = fDistribution.cumulative(range[1]) -
                            fDistribution.cumulative(range[0]);
                break;
            case GAMMA:
                distValue =
                    gammaCDF(range[1], (int) parameter[0], parameter[1]) -
                    gammaCDF(range[0], (int) parameter[0], parameter[1]);
                break;
            case GEOMETRIC:
                GeometricDistribution geometricDistribution = new
                        GeometricDistribution(parameter[0]);
                distValue = geometricDistribution.probability(range[0]) *
                            equalInd +
                            geometricDistribution.cumulative(range[1]) -
                            geometricDistribution.cumulative(range[0]);
                break;
            case HYPERGEOMETRIC:
                distValue =
                    hypergeometricCDF((int) range[0], (int) parameter[0],
                                      (int) parameter[1],
                                      (int) parameter[2]) * equalInd +
                    (hypergeometricCDF((int) range[1],
                                       (int) parameter[0],
                                       (int) parameter[1],
                                       (int) parameter[2]) -
                     hypergeometricCDF((int) range[0],
                                       (int) parameter[0],
                                       (int) parameter[1],
                                       (int) parameter[2]));
                break;
            case LOGNORMAL:
                ...
                distValue = lognormalDistribution.cumulative(range[1]) -
                            lognormalDistribution.cumulative(range[0]);
                break;
            case NEGATIVE_BINOMIAL:
                distValue = negativeBinomialCDF((int) range[1],
                                                (int) parameter[0],
                                                parameter[1]) -
                            negativeBinomialCDF((int) range[0],
                                                (int) parameter[0],
                                                parameter[1]);
                break;
            case PARETO:
                ...
                distValue = paretoDistribution.cumulative(range[1]) -
                            paretoDistribution.cumulative(range[0]);
                break;
            case POISSON:
                PoissonDistribution poissonDistribution =
                    new PoissonDistribution(parameter[0]);
                distValue = poissonDistribution.probability(range[0]) *
                            equalInd +
                            poissonDistribution.cumulative(range[1]) -
                            poissonDistribution.cumulative(range[0]);
                break;
            case T:
                TDistribution tDistribution =
                    new TDistribution((int) parameter[0]);
                distValue = tDistribution.cumulative(range[1]) -
                            tDistribution.cumulative(range[0]);
                break;
            case WEIBULL:
                WeibullDistribution weibullDistribution =
                    new WeibullDistribution(parameter[0]);
                distValue = weibullDistribution.cumulative(range[1]) -
     ...
     * @param r the location parameter.
     * @param lambda the scale parameter.
     * @return the cumulative distribution function.
     */

    public double gammaCDF(double x,
                           int r,
                           double lambda)
    {
        double cdf = 0;
        ...
        {
            for (double i = 0.001; i <= x; i = i + 0.001)
            {
                cdf += (double) (x / ((x / 0.001))) * pow(lambda, r) *
                       pow(i, r - 1) * exp( -lambda * i) / factorial(1, r - 1);
            }
            return cdf + (double) (x / (2 * (x / 0.001))) * pow(lambda, r) *
                   pow(0, r - 1) * exp( -lambda * 0) / factorial(1, r - 1);
    ...
    public double hypergeometricCDF(int x,
                                    int N,
                                    int r,
                                    int n)
    {
        ...
        if (x >= 0 && N >= 0)
        {
            ...
            {
                cdf = (combination(r, i) * combination(N - r, n - i)) /
                      combination(N, n) + cdf;
            }
        }
    ...
    public double negativeBinomialCDF(int x,
                                      int r,
                                      double p)
    {
        double cdf = 0;
        if (x >= 0 && (p >= 0.0 && p <= 1.0) && r >= 1)
        {
            for (int i = 0; i <= x; i++)
            {
                cdf = (combination(i + r - 1, i) * pow(p, r) *
                       pow(1.0 - p, i)) + cdf;
            }
        }
