package plots.javastat.regression.glm;
...
import plots.javastat.util.GLMDataManager;
import plots.javastat.util.Output;

import static plots.javastat.regression.glm.LinkFunction.*;
...
public class LogisticRegression extends GLMTemplate
{
                             ...
                             double[] ...covariate)
    {
        ...
        means = pi = means(link, coefficients, covariate);
                                        ...
                                        double[] ...covariate)
    {
        ...
        means = means(coefficients, covariate);
        responseVariance = responseVariance(means, ExponentialFamily.BINOMIAL);
                                         ...
                                         double[] ...covariate)
    {
        ...
        for (int i = 0; i < response.length; i++)
        {
            double m = means[i];
            residuals = Math.sqrt(
                    -2 * (response[i] * Math.log(m / (classSize[i] - m)) +
                          Math.log(classSize[i] - m)));
            if (response[i] - m > 0)
            {
