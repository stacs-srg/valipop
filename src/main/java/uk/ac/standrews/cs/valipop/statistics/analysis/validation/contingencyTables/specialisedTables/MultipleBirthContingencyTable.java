 ...
 */
public class MultipleBirthContingencyTable extends ContingencyTable {

    @SuppressWarnings({ "rawtypes", "unchecked" })
    public MultipleBirthContingencyTable(final ContingencyTree tree) {
        ...
        for (final Node leaf : tree.getLeafNodes()) {
            ...
            if (row != null) {
                ...
                try {
                    if (row.getVariable(LABEL_SOURCE).getValue().equals(SourceType.SIMULATED.toString()))
                        row.addDateVariable();
                    else
                        row.addDateVariable(-1);
                    ...
                    if (row.getVariable(LABEL_SEX).getValue().equals(SexOption.FEMALE.toString())) {
                        ...
                        row.deleteVariable(LABEL_PARTNER_AGE);
