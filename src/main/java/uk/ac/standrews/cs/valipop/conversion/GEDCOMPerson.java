...
public class GEDCOMPerson implements IPerson {
    ...
    private final GEDCOMImportAdapter adapter;
    protected int id;
    ...
    GEDCOMPerson(final Individual individual, final GEDCOMImportAdapter adapter) {
    ...
    private void setId(final Individual individual) {
        ...
        id = GEDCOMExportAdapter.idToInt(individual.getXref());
    }
    ...
    private void setParents(final Individual individual) {
        ...
        parent_id = families != null && !families.isEmpty() ? GEDCOMExportAdapter.idToInt(families.getFirst().getFamily().getXref()) : -1;
    }
    ...
    private void setPartnerships(final Individual individual) {
        ...
        if (familiesWhereSpouse != null)
            for (final FamilySpouse family : familiesWhereSpouse)
                partnershipIds.add(GEDCOMExportAdapter.idToInt(family.getFamily().getXref()));
    }
    ...
    private static LocalDate parseDate(final String date) {
        ...
        try {
            return LocalDate.parse(date, GEDCOMExportAdapter.FORMAT);
        }
        catch (final DateTimeParseException ignore) {

            // Deal with the case where the date string contains only a year.
            try {
                return LocalDate.parse("1 JUL " + date, GEDCOMExportAdapter.FORMAT);
            }
