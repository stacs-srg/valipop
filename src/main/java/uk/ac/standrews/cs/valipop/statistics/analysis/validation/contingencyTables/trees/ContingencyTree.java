 ...
 */
package uk.ac.standrews.cs.valipop.statistics.analysis.validation.contingencyTables.trees;

import uk.ac.standrews.cs.valipop.simulationEntities.IPerson;
import uk.ac.standrews.cs.valipop.simulationEntities.PopulationNavigation;
import uk.ac.standrews.cs.valipop.statistics.analysis.validation.contingencyTables.trees.doubleNodes.*;
import uk.ac.standrews.cs.valipop.statistics.analysis.validation.contingencyTables.trees.integerNodes.SourceNodeInt;
...
public class ContingencyTree extends Node<String, SourceType, Number, Number> {
    ...
    private SourceNodeInt simulatedNode;
    private SourceNodeDouble desiredNode = null;
            ...
            for (final IPerson person : population) {
                ...
                if (PopulationNavigation.aliveOnDate(person, lastDayOfPreviousYear) && PopulationNavigation.inCountryOnDate(person, lastDayOfPreviousYear)) {
                    ...
                    if (year.getYear() == firstYearToProcess.getYear())
                        processPerson(person, year, SourceType.TARGET);

                    if (year.getYear() >= firstYearToProcess.getYear())
                        processPerson(person, year, SourceType.SIMULATED);
                }
            }
        }
    ...
    public Collection<Node> getLeafNodes() {
        ...
        childNodes.addAll(simulatedNode.getLeafNodes());
        childNodes.addAll(desiredNode.getLeafNodes());
    ...
    private Node addChildA(final SourceType childOption) {
        ...
        if (childOption == SourceType.SIMULATED) {
            simulatedNode = new SourceNodeInt(SourceType.SIMULATED, this);
            return simulatedNode;
        } else {
            desiredNode = new SourceNodeDouble(SourceType.TARGET, this);
            return desiredNode;
        }
    }
    ...
    public Node getChild(final SourceType option) throws ChildNotFoundException {
        ...
        if (option == SourceType.SIMULATED)
            if (simulatedNode != null) return simulatedNode;
