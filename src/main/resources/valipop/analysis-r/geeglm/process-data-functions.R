...
clean_data <- function(dirty_data, round = TRUE, start, end) {
  data <- dirty_data
  if (round) {
...
clean_ob_data <- function(dirty_data, max_birthing_age, round = TRUE, start, end) {
  data <- clean_data(dirty_data, round, start = start, end = end)
  data <- data[which(data$Age >= 15), ]
  data <- data[which(data$Age <= max_birthing_age), ]
  return(data)
}

clean_mb_data <- function(dirty_data, max_birthing_age, round = TRUE, start, end)  {
  clean_data <- clean_ob_data(dirty_data, max_birthing_age, round, start = start, end = end)
