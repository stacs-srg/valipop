...
# This must match the name of the appropriate enum value defined in
# valipop.statistics.analysis.validation.contingencyTables.trees.SourceType.
SIMULATED <- "SIMULATED"

# These must match the names of the appropriate enum values defined in
# valipop.statistics.analysis.validation.contingencyTables.trees.SexOption.
FEMALE <- "F"
MALE <- "M"

# The variables used in the formulae below must match the string constants
# defined in valipop.statistics.analysis.validation.contingencyTables.tables.ContingencyTable.

run_death_analysis <- function(in_data) {
  return(
    tryCatch(
...
run_birth_analysis <- function(in_data) {
  return(
    tryCatch(
...
run_multiple_birth_analysis <- function(in_data) {
  return(
    tryCatch(
...
run_partnership_analysis <- function(in_data) {
  return(
    tryCatch(
...
run_analysis <- function(formula, in_data, correlation = "ar1") {
...
read_in_data <- function(path) {
  ...
  return(read.csv(path, sep = ",", header = TRUE))
}

clean_data <- function(data, start_year, end_year) {
...
add_cohort_ids_death <- function(in_data) {
  ...
  male_l <- max( # nolint: object_usage_linter.
    in_data[which(in_data$Sex == MALE), ]$YOB
  )
  female_l <- max( # nolint: object_usage_linter.
    in_data[which(in_data$Sex == FEMALE), ]$YOB
  )

  data_id <- within(in_data, {
    idvar <- ifelse(Source == SIMULATED,  # nolint: object_usage_linter.
                    ifelse(Sex == MALE,
...
death_analysis <- try(run_death_analysis(death_ids))
birth_analysis <- try(run_birth_analysis(birth_ids))
multiple_birth_analysis <- try(run_multiple_birth_analysis(multiple_birth_ids))
partnership_analysis <- try(run_partnership_analysis(partnership_ids))
