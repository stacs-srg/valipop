...
convertResultSummaryToJob <- function(result, priority, n, deterministic, seed, constantsSet, resultsDir, summaryResultsDir, recordFormat = NA, reason = NA, reqMemory = NA) {
  ...
  job <- data.frame(
            ...
            "summary results dir" = summaryResultsDir,
            "required memory" = 
              if(is.na(reqMemory))
            ...
            #"oversized geography factor" = result$oversized.geography.factor,
            check.names=F
            )
...
repeatJobDeterministically <- function(result, priority, constantsSet, resultsDir, summaryResultsDir,recordFormat = NA, reason = NA, reqMemory = NA) {
  job <- convertResultSummaryToJob(result, priority, 1, TRUE, result$Seed, constantsSet, resultsDir, summaryResultsDir, recordFormat = recordFormat, reason = reason, reqMemory = reqMemory)
  return(job)
}

repeatJobsDeterministically <- function(results, constantsSet, resultsDir, summaryResultsDir, recordFormat = NA, priority = 4, reason = NA, reqMemory = NA) {
  jobs <- data.frame()
  
  for (index in 1:nrow(results)) { 
    result = results[index, ]
    jobs <- rbind(jobs, repeatJobDeterministically(result, priority, constantsSet, resultsDir, summaryResultsDir, recordFormat, reason, reqMemory))
  }
    
  return(jobs)
}

runMorePromisingJobs <- function(results, constantsSet, uptoN, resultsDir, summaryResultsDir, cutoff, recordFormat = NA, priority = 4, reason = NA, reqMemory = NA) {
        ...
        if(uptoN - row$count > 0)
          jobs <- rbind(jobs, 
                        convertResultSummaryToJob(result, priority, uptoN - row$count, 
                                                  FALSE, "-", constantsSet, resultsDir, summaryResultsDir, recordFormat, reason, reqMemory))
      }
  }
  return(jobs)
}
...
searchNearPromisingJobs <- function(results, constantsSet, uptoN, resultsDir, summaryResultsDir, cutoff, minStep, explorationStep, priority = 4, recordFormat = NA, reason = NA, reqMemory = NA) {
