...
The initial size of preliminary population at [`initialisation_start`](../usage/configuration/config-reference.md#initialisation_start) is calculated by working backwards from [`target_initial_population_size`](../usage/configuration/config-reference.md#target_initial_population_size). ValiPop uses the setup birth rate [`initialisation_birth_rate`](../usage/configuration/config-reference.md#initialisation_birth_rate) and setup death rate [`initialisation_death_rate`](../usage/configuration/config-reference.md#initialisation_death_rate) to calculate the hypothetical population size at [`initialisation_start`](../usage/configuration/config-reference.md#initialisation_start) given [`target_initial_population_size`](../usage/configuration/config-reference.md#t0_pop0_size).

For example, configuring a larger setup birth rate than death rate would mean the population size at [`initialisation_start`](../usage/configuration/config-reference.md#initialisation_start) would be smaller to allow for it to grow and reach the size of [`target_initial_population_size`](../usage/configuration/config-reference.md#target_initial_population_size).

### Preliminary population initialisation

When then simulation begins at [`initialisation_start`], orphan children are spawned in to match the population size. At each timestep ([`simulation_time_step`](../usage/configuration/config-reference.md#simulation_time_step)), additional children will be spawned if the preliminary population is less than expected at that time. Notably, if the preliminary population is greater than expected (such as due to a setup higher death rate), ValiPop will not remove individuals and instead rely on the self correction of the distributions later on.
