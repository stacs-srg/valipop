...
Each input distribution of a property will apply for a period of time during the simulation. The length of the period is defined by the [`distribution_granularity`](config-reference#distribution_granularity) option in the config file. ValiPop will divide the given input distributions into these equal periods based on which input distribution is closest to the end time of that period.
